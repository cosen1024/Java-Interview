autoauto- [String相关](#string相关)auto    - [字符型常量和字符串常量的区别？](#字符型常量和字符串常量的区别)auto    - [什么是字符串常量池？](#什么是字符串常量池)auto    - [String str="aaa"与 String str=new String("aaa")一样吗？`new String(“aaa”);`创建了几个字符串对象?](#string-straaa与-string-strnew-stringaaa一样吗new-stringaaa创建了几个字符串对象)auto    - [String 是最基本的数据类型吗?](#string-是最基本的数据类型吗)auto    - [String有哪些特性?](#string有哪些特性)auto    - [在使用 HashMap 的时候，用 String 做 key 有什么好处？](#在使用-hashmap-的时候用-string-做-key-有什么好处)auto- [包装类型](#包装类型)auto    - [包装类型是什么？基本类型和包装类型有什么区别？](#包装类型是什么基本类型和包装类型有什么区别)auto    - [解释一下自动装箱和自动拆箱？](#解释一下自动装箱和自动拆箱)auto    - [int 和 Integer 有什么区别?](#int-和-integer-有什么区别)auto    - [两个new生成的Integer变量的对比](#两个new生成的integer变量的对比)auto    - [Integer变量和int变量的对比](#integer变量和int变量的对比)auto    - [非new生成的Integer变量和new Integer()生成变量的对比](#非new生成的integer变量和new-integer生成变量的对比)auto    - [两个非new生成的Integer对象的对比](#两个非new生成的integer对象的对比)auto- [反射](#反射)auto    - [什么是反射？](#什么是反射)auto    - [反射机制的优缺点有哪些？](#反射机制的优缺点有哪些)auto    - [如何获取反射中的Class对象？](#如何获取反射中的class对象)auto    - [Java反射API有几类？](#java反射api有几类)auto    - [反射使用的步骤？](#反射使用的步骤)auto    - [为什么引入反射概念？反射机制的应用有哪些？](#为什么引入反射概念反射机制的应用有哪些)auto    - [反射机制的原理是什么？](#反射机制的原理是什么)auto- [泛型](#泛型)auto    - [Java中的泛型是什么 ?](#java中的泛型是什么-)auto    - [使用泛型的好处是什么?](#使用泛型的好处是什么)auto    - [Java泛型的原理是什么 ? 什么是类型擦除 ?](#java泛型的原理是什么--什么是类型擦除-)auto    - [什么是泛型中的限定通配符和非限定通配符 ?](#什么是泛型中的限定通配符和非限定通配符-)auto    - [List<? extends T>和List <? super T>之间有什么区别 ?](#list-extends-t和list--super-t之间有什么区别-)auto    - [可以把List`<String>`传递给一个接受List`<Object>`参数的方法吗？](#可以把liststring传递给一个接受listobject参数的方法吗)auto    - [Array中可以用泛型吗?](#array中可以用泛型吗)auto    - [判断`ArrayList<String>`与`ArrayList<Integer>`是否相等？](#判断arrayliststring与arraylistinteger是否相等)auto- [序列化](#序列化)auto    - [Java序列化与反序列化是什么？](#java序列化与反序列化是什么)auto    - [为什么需要序列化与反序列化？](#为什么需要序列化与反序列化)auto    - [序列化实现的方式有哪些？](#序列化实现的方式有哪些)auto        - [**Serializable**接口](#serializable接口)auto        - [**Externalizable**接口](#externalizable接口)auto        - [两种序列化的对比](#两种序列化的对比)auto    - [什么是serialVersionUID？](#什么是serialversionuid)auto    - [为什么还要显示指定serialVersionUID的值?](#为什么还要显示指定serialversionuid的值)auto    - [serialVersionUID什么时候修改？](#serialversionuid什么时候修改)auto    - [Java 序列化中如果有些字段不想进行序列化，怎么办？](#java-序列化中如果有些字段不想进行序列化怎么办)auto    - [静态变量会被序列化吗?](#静态变量会被序列化吗)auto- [异常](#异常)auto    - [Error 和 Exception 区别是什么？](#error-和-exception-区别是什么)auto    - [非受检查异常(运行时异常)和受检查异常(一般异常)区别是什么？](#非受检查异常运行时异常和受检查异常一般异常区别是什么)auto    - [throw 和 throws 的区别是什么？](#throw-和-throws-的区别是什么)auto    - [NoClassDefFoundError 和 ClassNotFoundException 区别？](#noclassdeffounderror-和-classnotfoundexception-区别)auto    - [Java常见异常有哪些？](#java常见异常有哪些)auto    - [try-catch-finally 中哪个部分可以省略？](#try-catch-finally-中哪个部分可以省略)auto    - [try-catch-finally 中，如果 catch 中 return 了，finally 还会执行吗？](#try-catch-finally-中如果-catch-中-return-了finally-还会执行吗)auto    - [JVM 是如何处理异常的？](#jvm-是如何处理异常的)auto- [IO](#io)auto    - [Java的IO 流分为几种？](#java的io-流分为几种)auto    - [字节流如何转为字符流？](#字节流如何转为字符流)auto    - [字符流与字节流的区别？](#字符流与字节流的区别)auto    - [BIO、NIO、AIO的区别？](#bionioaio的区别)auto    - [Java IO都有哪些设计模式？](#java-io都有哪些设计模式)auto- [参考](#参考)autoauto
## String相关
